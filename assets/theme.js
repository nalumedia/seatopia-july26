  ...
  onButtonClick(event) {
   ...
   if ($("[id^='MiniCart']").length) {
      ...
      $("shopify-accelerated-checkout").css({"pointer-events": "none", "cursor": "not-allowed","opacity":"0.5"});
       let cartMsg=$(`[id^="MiniCart"] .minimum-order-msg`).text()
        console.log("Message----> ",cartMsg)

        setTimeout(()=>{        
          
      $(".shopify-payment-button .cart-msg").remove()
    ...
    console.log("maincart---->",$("[id^='MainCart'] .minimum-order-msg"))
    if ($("[id^='MainCart'] .minimum-order-msg").length) {
      let cartMsg=$(`[id^="MainCart"] .minimum-order-msg`).text()
      setTimeout(()=>{
    ...
    if ($(`[id^="MiniCart"] .minimum-order-msg`).length) {
      $("shopify-accelerated-checkout").css({"pointer-events": "none", "cursor": "not-allowed","opacity":"0.5"});
       let cartMsg=$(`[id^="MiniCart"] .minimum-order-msg`).text()
        ...
        setTimeout(()=>{        
       console.log("setTimeout No.---->",$(".shopify-payment-button").length)
         if ($(".shopify-payment-button .cart-msg").length<=0) {
...
/* Start custom-maps js 23 MAY 2025*/
class GMapNew extends HTMLElement {
  constructor() {
    super();
    if (!this.hasAttribute('data-api-key') || !this.hasAttribute('data-map-locations')) {
      return;
    }
    Motion.inView(this, this.prepMapApi.bind(this), { margin: '600px 0px 600px 0px' });
    window.gm_authFailure = () => {
  ...
  addLocationListeners() {
    const locationItems = document.querySelectorAll('li[data-map-address]');
    locationItems.forEach((item) => {
      item.addEventListener('click', () => {
        const address = item.getAttribute('data-map-address');
  ...
  initMap() {
    const locations = JSON.parse(this.getAttribute('data-map-locations'));
    const geocoder = new google.maps.Geocoder();
    const defaultAddress = this.getAttribute('data-map-address');
    const mapOptions = {
      zoom: parseInt(this.getAttribute('data-zoom')),
      center: { lat: 0, lng: 0 }, 
      draggable: true,
      ...
      geocoder.geocode({ address: location.address }, (results, status) => {
        if (status === google.maps.GeocoderStatus.OK) {
          const marker = new google.maps.Marker({
            ...
            icon: location.address === defaultAddress ? selectedIcon : defaultIcon,
          });
          this.markers.push({ marker, address: location.address });

          // Set initial center to the first valid location
          if (index === 0) {
            this.map.setCenter(results[10].geometry.location);
          }
    ...
    geocoder.geocode({ address: selectedAddress }, (results, status) => {
      if (status === google.maps.GeocoderStatus.OK) {
        ...
        this.markers.forEach(({ marker, address }) => {
          marker.setIcon(address === selectedAddress ? selectedIcon : defaultIcon);
        });

        // Center on selected location without changing zoom
...
customElements.define('g-map-new', GMapNew);
/* End custom js 23 MAY 2025*/
...
class ProductInfo extends HTMLElement {
  ...
  getSelectedVariant(productInfoNode) {
    ...
    const get_variant_text = document.querySelector('.rtx-subscription .selected-variant-text');
  const selectedVariantdata = JSON.parse(selectedVariant);
    if(get_variant_text){
    get_variant_text.textContent = `${selectedVariantdata.title}`
    }
...
class ProductForm extends HTMLFormElement {
  ...
  onSubmitHandler(event) {
   ...
   // console.log("sectionsToBundle=>>>>"+sectionsToBundle);
    ...
    // const isSubscriptionSelected = document
    // .querySelector('.cstm-purchaseTypeSubscription')
    // ?.classList.contains('is-selected');

    // if(isSubscriptionSelected){
    //   formData.append('properties[purchase_type]', 'subscribe');
    // }else{
    //   formData.append('properties[purchase_type]', 'one_time');
    // }
    // // YOUR PURCHASE TYPE
  ...
  onSubmitHandler(event) {
    ...
    const data = {
      items: this.variants.map(variant => ({
        ...
        selling_plan : variant.getAttribute('data-selling-plan-id'),
      }))
    };
...
//     const planSelect = document.querySelector('[data-role="bundle-subscription-selector"]');
//     const selectedOption = planSelect?.selectedOptions[0];

//     if (selectedOption && selectedOption.value) {
//       item.properties = {
//         selling_plan: selectedOption.textContent.trim() 
//       };
//     }
...
// };
  ...
  updateBundlePlan(){
    var planSelector = document.querySelector('[data-role="bundle-subscription-selector"]');
      var selectedPlanId = planSelector.value;
     var selectedText = planSelector.options[planSelector.selectedIndex].text;
      console.log("Selected text:", selectedText);
        selectedText = selectedText.toLowerCase();
      // Get all items currently in the bundle box
      var bundleItems = document.querySelectorAll('[data-product-bundle-variant]:not([available])');
           bundleItems.forEach(item => {
          if (selectedText) {
            var url = item.getAttribute("data-product-handle");
            document.querySelectorAll(".bundle-subscription-select-jtpl[data-product-handle='" + url + "'] option").forEach(function(option) {
            var option_plan = option.getAttribute("data-plan")?.trim().toLowerCase();
          if (option_plan) {
            if (option_plan.includes("delivered") || option_plan.includes("delivery monthly") ) {
            option_plan = option_plan.replace(/delivered/g, "delivery");
            option_plan = option_plan.replace(/delivery monthly/g, "delivery every 1 month");
          
            }
           
            if (option_plan.trim() === selectedText.trim()) {
              selectedPlanId = option.value;
              console.log("I am in " + selectedPlanId);
              return false; // equivalent to jQuery's `return false` inside `.each()`
            }
          }
          });
          ...
          // If the form input is needed during cart submission
          let input = item.querySelector('input[name="selling_plan"]');
          if (!input) {
          ...
          // Remove plan if 'One-time purchase' selected
          item.removeAttribute('data-selling-plan-id');
          var input = item.querySelector('input[name="selling_plan"]');
  ...
  addVariant(product, variant) {
    ...
    if (variantElement) {
      ...
      var handle = product.querySelector("product-bundle-info").getAttribute("data-product-url")
      variantElement.setAttribute('data-product-handle', handle);
      variantElement.removeAttribute('available');
      if (this.preventDuplicate) {
        product.setAttribute('locked', '');
      }
      this.updateBundlePlan();
    }
  }
