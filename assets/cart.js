...
class CartItems extends HTMLElement {
  ...
  constructor() {
    ...
    this.cartUpdateUnsubscriber = theme.pubsub.subscribe(theme.pubsub.PUB_SUB_EVENTS.cartUpdate, this.onCartUpdate.bind(this));
     // Initial load
        fetch('/cart.js')
          .then(res => res.json())
          .then(cart => this.updateFreeShippingMessage(cart));
  ...
  onChange(event) {
    ...
    this.checkThresholdAmount(event);
  }
...
updateFreeShippingMessage(cart) {
  ...
  const codes = message.dataset.freeShippingCodes
    .toUpperCase()
    .replace(/\s/g, "")
    .split("|");
  ...
  const matched = cart.discount_codes?.some(discount =>
    discount.applicable &&
    codes.includes(discount.code.toUpperCase())
  );

  const messageHTML = message.dataset.message;
const appliedCode = cart.discount_codes?.find(discount => discount.applicable);

if (appliedCode) {
  message.innerHTML = messageHTML.replace(
    /<CODE>/gi,
    appliedCode.code
  );
}

  document.querySelectorAll(".free-shipping-message").forEach((el) => {
  ...
  onCartUpdate(event) {
    ...
    if (mainCart) {
      ...
      if (updatedElement) {
        ...
        console.log("------>cartpage",updatedElement.querySelector(".minimum-order-msg"))
    ...
    this.updateFreeShippingMessage(event.cart);
  ...
  checkThresholdAmount(e){
    console.log("checkThresholdAmount------>")
  }
